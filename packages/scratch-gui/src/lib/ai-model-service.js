...
// MediaPipe's LlmInference cannot run two generateResponse calls at once, so all
// generations (including the load-time verification chat) are serialized through
// this promise chain. Without this, the first user generation can collide with the
// in-flight verification chat and come back empty.
let _genQueue = Promise.resolve();

const _enqueueGenerate = prompt => {
    const result = _genQueue.then(() => _llmInference.generateResponse(prompt));
    _genQueue = result.catch(() => {}); // keep the queue alive after a failure
    return result;
};
...
const loadModel = async url => {
    ...
    try {
        ...
        // Verification chat — non-blocking, but queued so it can't overlap a real
        // generation that the user kicks off as soon as the model reports ready.
        _enqueueGenerate('Say exactly: Hello from Gemma')
            .then(response => {
                // eslint-disable-next-line no-console
...
const generate = prompt => {
    ...
    return _enqueueGenerate(prompt);
};
