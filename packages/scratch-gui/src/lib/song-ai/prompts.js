...
const buildEditSystemPrompt = () => [
    ...
    '  1. A HARMONY + STRUCTURE BRIEF derived from the existing notes: the implied',
    '     chord for each region, which bars each part plays vs. rests (the track',
    '     you are editing is marked), and each part\'s register. Treat it as ground',
    '     truth about the song.',
    '  2. The full song JSON: tempo, lengthSteps, stepsPerBeat=4, and all tracks.',
    '  3. trackIndex: which track in song.tracks the user wants to edit.',
    '  4. The user\'s edit request in natural language.',
    '',
    ...
    '- FOLLOW THE HARMONY + STRUCTURE BRIEF above. For each region it lists, put the',
    '  notes on STRONG BEATS (steps 0, 4, 8, 12 of each bar) on that region\'s listed',
    '  chord tones; use other in-scale tones on weak beats. Regions marked "(unclear',
    '  — stay in scale)" have no clear chord — just stay in scale and lean on the',
    '  region\'s bass note there.',
...
const buildGenerateTrackSystemPrompt = kind => [
    ...
    'You will receive (in the user message):',
    '  1. A HARMONY + STRUCTURE BRIEF derived from the existing notes: the implied',
    '     chord for each region, which bars each existing part plays vs. rests, and',
    '     the register each part occupies. Treat it as ground truth about the song.',
    '  2. The full song JSON: tempo, lengthSteps, stepsPerBeat=4, and the existing tracks.',
    `  3. The kind of new track to add: ${kind}.`,
    '  4. The user\'s description of what the new track should sound like.',
    '',
    ...
    'Harmony — fit the existing song:',
    '- FOLLOW THE HARMONY + STRUCTURE BRIEF above. For each region it lists, put the',
    '  notes that fall on STRONG BEATS (steps 0, 4, 8, 12 of each bar) on that',
    '  region\'s listed chord tones; other in-scale tones are for weak beats. Regions',
    '  marked "(unclear — stay in scale)" have no clear chord — just stay in scale',
    '  and lean on the region\'s bass note there.',
    '- FOLLOW THE STRUCTURE GRID: prefer entering where existing parts REST, do NOT',
    '  double the rhythm of a part already playing in that bar, and place your part',
    '  in a register the brief shows as open for its role.',
    '- Land notes on CHORD TONES on strong beats; use passing tones, neighbor',
    '  tones, and suspensions on weak beats. Stay in the song\'s key.',
    '- Borrow freely from classical (functional cadences, voice-leading),',
    '  jazz (7th-chord arpeggios, chromatic approach tones, ii-V-I shapes),',
